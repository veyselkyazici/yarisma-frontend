...
export const fetchSaveUserProfile = createAsyncThunk(
    ...
    async(user)=>{
      const result = await  fetch('http://34.69.208.110:9092/v1/api/user/newcreateuser',{
        ...
        .then().catch((err)=>console.log('hata oldu....: ',err))
        return result;
    }
);
...
export const fetchGetAllUsers = createAsyncThunk(
    ...
    async(currentPage)=>{
      const result=
       await fetch(`http://34.69.208.110:9092/v1/api/user/findallslice?currentPage=${currentPage}&pageSize=8&sortParameter=id&direction=desc`)
        .then(p=>p.json())        
